...
{{/*
Return CN for auto generated ingress secret
*/}}
{{- define "om-chart.ingress-cn" -}}
{{- $hostname := "" }}
{{- if (.Capabilities.APIVersions.Has "route.openshift.io/v1") }}
{{- $hostname = printf ".%s.%s" .Release.Name .Values.appserver.ingress.host }}
{{- else }}
{{- $hostname = .Values.appserver.ingress.host }}
{{- end }}
{{- if or (.Capabilities.APIVersions.Has "route.openshift.io/v1") (gt (len $hostname) 63) }}
{{- printf "*%s" ($hostname | trunc -63) }}
{{- else }}
{{- printf "%s" $hostname }}
{{- end }}
{{- end -}}